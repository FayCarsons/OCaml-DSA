  ...
  Array.iteri
    (fun guess_index -> function
      | Some (turn, markers) ->
        ...
        Printf.printf "Turn %d: [ %s; %s; %s; %s ]\n" guess_index one two three four;
...
let print_code code =
  print_string "The code was:\n[ ";
  Array.iter (fun color -> print_string @@ color_to_string color ^ "; ") code;
  print_string "]\n"
;;
...
let check_guess code guess =
  ...
  let rec go seen = function
    ...
    | n ->
      ...
      else if Array.mem guessed_color code && not (ColorSet.mem guessed_color seen)
      then response.(n) <- Some White;
      go (ColorSet.add guessed_color seen) (succ n)
  in
  go ColorSet.empty 0
;;
...
let rec update ({ guesses; turns; code } as game) =
  ...
  then (
    ...
    match read_line () |> String.trim |> parse_guess with
    | Ok guess ->
      let response = check_guess code guess in
      if Array.for_all (Util.is_some_and (( = ) Black)) response
      then (
    ...
    print_endline "You are not the mastermind :/";
    print_code code)
;;
